...
class App extends Component {
  ...
  constructor() {
    ...
    this.state = {
	   ...
	   items: {} // the property names of this object are the names of the lists; their values are arrays of the items in each list
    };
  }
  ...
  handleAddList(s) {
      this.setState(
        {lists : this.state.lists.concat(s.listName)}, 
        () => this.state.items[s.listName] = []
      );
  }
  ...
  handleAddItem(s) {
      this.setState(
        () => this.state.items[s.listName].push(s.newItem)
      );
  }

  /**
   * Renders two child components: AddList and Lists.
   */
  render() {
    return (
      <div className="App">
